...
Prepod::Prepod(const Prepod &other)
    : Human(other), degree(other.degree), position(other.position), works(other.works) {}
Prepod::~Prepod() {}

std::string Prepod::get(std::string param) const {
    if (param == "degree") return degree;
    if (param == "position") return position;
    if (param == "works") return works;
    return Human::get(param);
}

void Prepod::set(std::string param, std::string value) {
    if (param == "degree") degree = value;
    else if (param == "position") position = value;
    else if (param == "works") works = value;
