...
public:
  ...
  {
    BinTreeException::throwIfElementExists(findRec(root, value) != nullptr,
                                           "добавления");
  ...
  {
    BinTreeException::throwIfElementNotFound(findRec(root, value) != nullptr,
                                             "удаления");
  ...
  {
    BinTreeException::throwIfElementNotFound(findRec(root, oldValue) != nullptr,
                                             "обновления");
    Node<T> *node = findRec(root, oldValue);

    if (oldValue == newValue)
      return false;

    BinTreeException::throwIfElementExists(
        findRec(root, newValue) != nullptr,
        "обновления (новое значение уже существует)");
