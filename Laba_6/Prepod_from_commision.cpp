...
std::ostream &operator<<(std::ostream &os, const Prepod_from_commision &s)
{
  if (&os == &std::cout)
  {
    ...
    os << std::left << std::setw(20) << s.Prepod::get("degree") << std::setw(15)
       ...
       << std::setw(25) << s.get("commision_works");
  }
  else
  {
    // File output
    os << static_cast<const Human &>(s) << " " << s.Prepod::get("degree") << " "
       << s.Prepod::get("position") << " " << s.Prepod::get("works") << " "
       << s.Member_of_commision::get("commision_name") << " "
       << s.Member_of_commision::get("biography") << " "
       << s.get("commision_works");
  }
  return os;
}
...
std::istream &operator>>(std::istream &is, Prepod_from_commision &pfc)
{
  ...
  std::string temp_c_works = InputValidator::readSingleWord(
